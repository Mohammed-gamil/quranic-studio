...
export default function VerseSelectorView({ 
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-20 animate-fade-in text-[var(--color-primary)]">
      {/* Header */}
      <header className="border-b-2 border-[var(--color-primary)] pb-6 md:pb-8 mb-8 md:mb-12 flex flex-col gap-4 relative">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2.5 md:p-3 border-2 border-[var(--color-primary)] hover:bg-[var(--color-surface-dim)] transition-all shadow-[2px_2px_0px_0px_var(--color-primary)] active:shadow-none active:translate-y-[1px] shrink-0">
          ...
          <div className="flex flex-col gap-1 min-w-0">
            ...
            <h1 className="font-serif italic text-2xl md:text-4xl leading-none tracking-tight">{surah.nameAr} <span className="text-base md:text-xl opacity-30 not-italic ml-1 md:ml-2">/ {surah.nameEn}</span></h1>
          </div>
        </div>

        {/* Reciter dropdown — full width on mobile */}
        {reciters.length > 0 && (
          <div className="flex flex-col gap-1 bg-[var(--color-surface-dim)] border border-[var(--color-primary)]/10 p-3 shadow-sm select-none">
            ...
            <select
              ...
              className="w-full bg-white border border-[var(--color-primary)]/20 px-3 py-2 font-mono text-[11px] text-[var(--color-primary)] focus:outline-none focus:border-[var(--color-primary)] cursor-pointer font-bold"
            >
              {reciters.map(rec => (
      ...
      {!loading && (
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 mb-8 md:mb-10 pb-4 md:pb-6 border-b border-[var(--color-primary)]/10">
          ...
          <div className="w-full md:w-auto flex items-center gap-4 bg-[var(--color-surface-dim)] border-2 border-[var(--color-primary)] px-4 md:px-6 py-2.5 font-mono text-[10px] uppercase tracking-widest font-bold">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 md:gap-4">
          {filteredVerses.map((ayah) => {
            ...
            return (
              <div key={ayah.number} onClick={() => handleSelectVerse(ayah.number)} className={`group p-4 md:p-8 border-2 transition-all cursor-pointer relative overflow-hidden flex items-start gap-4 md:gap-8 ${isSelected ? 'bg-white border-[var(--color-primary)] shadow-[8px_8px_0px_0px_var(--color-primary)] scale-[1.01] z-10' : 'bg-[var(--color-surface-dim)] border-[var(--color-primary)]/10 hover:border-[var(--color-primary)]'}`}>
                {isSelected && <IslamicCornerOrnament position="top-right" className="w-32 h-32 -right-4 -top-4 opacity-[0.03]" />}
                
                <div className={`w-5 h-5 md:w-6 md:h-6 border-2 border-[var(--color-primary)] mt-2 flex items-center justify-center transition-colors shrink-0 ${isSelected ? 'bg-[var(--color-primary)] text-white' : 'bg-white'}`}>
                  {isSelected && <Check className="w-3 h-3 md:w-4 md:h-4 stroke-[3]" />}
                </div>

                <div className="flex-1 flex flex-col gap-4 md:gap-6 min-w-0">
                  <div className="flex justify-between items-start gap-4 md:gap-10">
                    <p className="flex-1 text-right font-serif text-2xl sm:text-3xl md:text-4xl leading-[1.8] text-[var(--color-primary)]" dir="rtl">{ayah.text}</p>
                    <span className="w-8 h-8 md:w-10 md:h-10 border-2 border-[var(--color-primary)] bg-white flex items-center justify-center font-mono text-xs font-bold shrink-0">{toArabicNumber(ayah.number)}</span>
                  </div>
                  <p className="font-sans text-[var(--color-primary)]/60 text-sm md:text-base leading-relaxed max-w-[90%] border-t border-[var(--color-primary)]/10 pt-3 md:pt-4 italic">{ayah.translation}</p>
                </div>
              </div>
            );
          })}
